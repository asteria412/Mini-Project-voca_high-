...
# -----------------------------------------------------------------------------
# [HSK 5급 작문 엔진] 2025년 최신 출제 경향(딥리서치) 및 전략 통합 모듈
# -----------------------------------------------------------------------------
...
# =============================================================================
# [PART 2] 100번 문제 생성기 (4대 빈출 테마 전략)
# =============================================================================
...
def evaluate_writing_v2(mode, user_input, ref_data):
    ...
    **1단계: 기본 등급 결정 (Base Score)**
    - 조건: 5개 단어 사용(99번) / 그림 연관성 있음(100번).
    - **CASE A (모범):** 어법 오류가 없고 + 내용이 논리적이다 -> **[상] 등급 (기본 80점) 부여**.
    - **CASE B (부족):** 어법 오류가 있거나 + 문장이 너무 단순하다 -> **[중] 등급 (70점) 부여**.
    - **CASE C (미달):** 필수 단어 누락 / 그림과 무관 / 심각한 비문 -> **[하] 등급 (50점) 부여**.

    **2단계: 고득점 승격 심사 (Bonus for 85~100)**
    - 1단계에서 [상] 등급(80점)을 받은 경우에만 적용합니다. 다음 요소가 있을 때마다 점수를 추가하세요.
      ① **접속사 사용:** (예시: 因此, 不仅...而且..., 即使 등) 논리적 연결이 매끄러움.
      ② **복문 구조:** (예시: 把자문, 被자문, 관계절 등) 문장 구조가 다양함.
      ③ **고급 어휘:** (예시: 显然, 逐渐, 各种各样 등) 5급 필수 어휘나 성어를 적절히 사용함.
      ④ **내용의 완결성:** 서론-본론-결론(서사)이 자연스러움.

    **3단계: 논리 검증 및 강등 (Safety Check)**
    - 문법이 맞아도 **'비상식적 내용(예: 빨간 사자, 방을 열애하다)'**이 발견되면 -> **무조건 [중] 등급(75점 이하)으로 강등**시키거나 10점을 감점하세요.
    - 99번에서 단어를 억지로 끼워 맞췄거나, 100번에서 그림과 무관한 내용이 절반 이상이면 감점하세요.

    [출력 형식 - JSON Only]
    {{
        "score": 85 (0~100점 사이 정수),
        "correction": "문법 교정 및 고급 표현을 추가한 모범 답안 (중국어)",
        "translation": "모범 답안의 한국어 해석",
        "explanation": "어떤 로직으로 점수가 산정되었는지 설명. (예: '기본기는 완벽하여 80점 시작했습니다. 접속사와 복문을 사용하여 +5점 되었습니다.')",
        "better_expression": "점수를 더 올리기 위해 추천하는 5급 고급 어휘나 접속사"
    }}
    """
